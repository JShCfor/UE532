...
void UPrimitiveComponent::SetDasStencilValue(int32 Value)
{
	if (DasStencilValue != Value)
	{
		DasStencilValue = Value;
		if (SceneProxy)
		{
			SceneProxy->SetDasStencilValue_GameThread(DasStencilValue);
		}
	}
}

void UPrimitiveComponent::SetDasCustomValue(int32 Value)
{
	if (DasCustomValue != Value)
	{
		DasCustomValue = Value;
		if (SceneProxy)
		{
			SceneProxy->SetDasCustomValue_GameThread(DasCustomValue);
		}
	}
}

void UPrimitiveComponent::SetDasCustomAttribute(const TMap<FString, FString>& mapAttributes)
{
	DasCustomAttributes = mapAttributes;
	if (SceneProxy)
	{
		SceneProxy->SetDasCustomAttribute_GameThread(DasCustomAttributes);
	}
}
